...
class BO_algo(object):
    def __init__(self):
        ...
        RBF1 =  ConstantKernel(1.5) * RBF(length_scale=1.5)
        RBF2 =  ConstantKernel(3.5) * RBF(length_scale=2.0)
        ...
        self.constraint_model =  GaussianProcessRegressor(kernel=RBF2) # Following the description from the hw
        self.objective_model =  GaussianProcessRegressor(kernel=RBF1) # Following the description from the hw
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        '''
        return sig_obj * (z_obj * scipy.stats.norm.cdf(z_obj) + scipy.stats.norm.pdf(z_obj)) * p0

        # reference: http://proceedings.mlr.press/v32/gardner14.pdf
