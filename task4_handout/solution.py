...
#        self.path_start_idx = self.ptr
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                            phi=self.phi_buf, logp=self.logp_buf, rew=self.rew_buf)
#        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
...
class Agent:
    ...
    def pi_update(self, data, gamma, lam):
        """
        ...
        # mean over returns
        mor = np.zeros_like(ret)
        for i in range(len(mor)):
            mor[i] = 1/(i+1) * torch.sum(ret[0:i+1])
    ...
    def v_update(self, data, gamma, lam):
        """
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            self.pi_update(data,gamma,lam)
            self.v_update(data,gamma,lam)
