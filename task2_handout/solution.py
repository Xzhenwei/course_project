...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        assert not any(True for _ in self.prior_bias.parameters()), 'Prior cannot have parameters'
...
#            self.bias_var_posterior = None
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        weight = self.weights_var_posterior.sample()
        log_prior = self.prior_weight.log_likelihood(weight)
        log_variational_posterior = self.weights_var_posterior.log_likelihood(weight)
        bias = None
        if self.use_bias:
            s_bias = self.bias_var_posterior.sample()
            log_prior += self.prior_bias.log_likelihood(s_bias)
            log_variational_posterior += self.bias_var_posterior.log_likelihood(s_bias)
        weight = weight.reshape(self.out_features, self.in_features)
        return F.linear(inputs, weight, s_bias), log_prior, log_variational_posterior
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, layer in enumerate(self.layers):
            feat, log_like, log_var_post = layer.forward(current_features)
            new_features = feat
            log_prior += log_like
            log_variational_posterior += log_var_post
